...
#define JOYANALOGCC 2
#define JOYDIGITALCC 3
...
byte joyMode = PITCHBAND; // режим джойстика 
...
*/
// автомат обработчик вертикального перемещения джойстика
  switch(joystickX){
	  case 0:
	  joystickXMs = ms;
	  ...
	  break;
	  case 5:
	  // выберем в каком режиме работаем
	  
	  if(joyMode == PITCHBAND){
		  joystickX = 10;// GO в работу режима питч
	  } else if (joyMode == JOYANALOGCC){
		  joystickX = 20;// GO в работу аналогового режима режима
	  } else if (joyMode == JOYDIGITALCC) joystickX = 30;// GO в работу цифрового режима
	  break;
	  case 10:
		  // режим ПИТЧБЭНД
		joyXanalog.update();
		...
		//  ЕСЛИ ОПРОС ШИФТА УСПЕШЕН, ТО УХОДИМ В РЕЖИМ ШИФТ ПИТЧ
	  break;
	  case x:
	  if((ms - joystickXMs) > joystickXMsDel){
		joystickXMs = ms;
